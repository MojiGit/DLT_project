...
contract forward{
    ...
    if (long[_address]) {
        updateMarginHelper(_address, counterParty, currentPrice - strike);
    } else if (short[_address]) {
        updateMarginHelper(_address, counterParty, strike - currentPrice);
    }
    }

    function updateMarginHelper(address _address, address _counterParty, uint256 priceDifference) internal {
        profit[_address] = int256(priceDifference * nominal) * int256(decimals);
        margin[_address] = int256(deposit[_address]) + profit[_address];

        profit[_counterParty] = -profit[_address];
        margin[_counterParty] = int256(deposit[_counterParty]) + profit[_counterParty];
    }
