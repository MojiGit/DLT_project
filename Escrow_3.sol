...
contract escrow {
    ...
    //defining the instance to use Mod contract
    mod public modContract;

    //defining the users that can interact with the contract
    address public owner;
    address public modAddress;
    address public constant me = 0x90512Af0B4d88C91b344A7F359CF35c7F83e948f;
    ...
    modifier onlyMe {
        ...
        require(msg.sender == me, "You are not allowed");
        _;
    }
    ...
    modifier allowedToRelease {    
        ...
        require(msg.sender == owner || msg.sender == modAddress, "You are not allowed");
        _;
    }

    modifier allowedToCancel {
        //only the counterparty or the mod can cancel the transaction
        require(msg.sender == counterparty[owner] || msg.sender == modAddress, "You are not allowed");
        _;
    }

    function definemod (address _modAddress) public onlyMe {
        modAddress = _modAddress;
        modContract = mod(_modAddress);
    }
    ...
    function release() public allowedToRelease {
        ...
        if (arbitrage == true) {
            ...
            payable (modAddress).transfer((value[owner] * commission) / 100);
            value[owner] = 0;
            modAddress = owner = address(0);
        } else {
